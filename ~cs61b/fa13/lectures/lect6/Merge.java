...
class Merge {
    ...
    public static int[] merge1(int[] A, int[] B) {
        return merge1(A, 0, A.length - 1, B, 0, B.length - 1);
    }

    /** Return the merge of A[L0..U0] and B[L1..U1] assuming A and B sorted. */
    static int[] merge1(int[] A, int L0, int U0, int[] B, int L1, int U1) {
        int N = U0 - L0 + U1 - L1 + 2;
        int[] C = new int[N];
        if (U0 < L0) arraycopy(B, L1, C, 0, N);
        else if (U1 < L1) arraycopy(A, L0, C, 0, N);
        else if (A[L0] <= B[L1]) {
            C[0] = A[L0]; arraycopy(merge1(A, L0 + 1, U0, B, L1, U1), 0,
                                    C, 1, N - 1);
        } else {
            C[0] = B[L1]; arraycopy(merge1(A, L0, U0, B, L1 + 1, U1), 0,
                                     C, 1, N - 1);
        }
        return C;
    }
    ...
    public static int[] merge2(int[] A, int[] B) {
        return merge2(A, 0, A.length - 1, B, 0, B.length - 1,
                      new int[A.length + B.length], 0);
    }

    /** Return the merge A[L0..U0] and B[L1..U1] into C[K...], assuming
     *  A and B sorted. */
    static int[] merge2(int[] A, int L0, int U0, int[] B, int L1, int U1,
                        int[] C, int k) {
        if (U0 < L0) {
            arraycopy(B, L1, C, k, U1 - L1 + 1);
        } else if (U1 < L1) {
            arraycopy(A, L0, C, k, U0 - L0 + 1);
        } else if (A[L0] <= B[L1]) {
            C[k] = A[L0];
            merge2(A, L0 + 1, U0, B, L1, U1, C, k + 1);
        } else {
            C[k] = B[L1];
            merge2(A, L0, U0, B, L1 + 1, U1, C, k + 1);
        }
        return C;
    }
